...
|---|---|---|---|
| TP-NSA-008 | Max UE TCP UL Throughput (LTE+NR) | 5G-NSA | Throughput_5G-NSA |

<div class="tc-content-border-box" markdown="1">

### Purpose {: .tc-shaded-header }

Verify maximum aggregate TCP uplink throughput for the maximum number of UEs in LTE+NR.

### Test Procedure {: .tc-shaded-header }

1. Make each 32 UE upload 1GB file from the FTP serever  
2. Measure aggregated average 32UE FTP upoad throughput for 2 min  
3. Repeat test step "2" five times
...
Stdev of (LTE+NR) PHY UL Throughput [Mbps]  
Stdev of (LTE+NR) RLC UL Throughput [Mbps]  
Stdev of (LTE+NR) PDCP UL Throughput [Mbps]  
Stdev of APP UL Throughput [Mbps]  
Technology
