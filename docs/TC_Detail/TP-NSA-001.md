...
|---|---|---|---|
| TP-NSA-001 | DL Throughput (LTE+NR) | 5G-NSA | Throughput_5G-NSA |

<div class="tc-content-border-box" markdown="1">

### Purpose {: .tc-shaded-header }

Verify maximum downlink throughput for a single UE in LTE+NR (EN-DC) environment.

### Test Procedure {: .tc-shaded-header }

1. UE download using iperf command  
2. Average download throughput measurement for 10 minutes
...
(LTE) PHY DL Throughput [Mbps]  
(LTE) RLC DL Throughput [Mbps]  
(LTE) PDCP DL Throughput [Mbps]  
(NR) PHY DL Throughput [Mbps]  
(NR) RLC DL Throughput [Mbps]  
(NR) PDCP DL Throughput [Mbps]  
